...
function Chat({ go, profile, otherUser }) {
  ...
  const send = async () => {
    ...
    const { data, error } = await supabase
      .from("messages")
      .insert({ sender_id: profile.id, recipient_id: otherUser.id, content })
      .select()
      .single();
    if (!error && data) {
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} style={{ alignSelf: mine ? "flex-end" : "flex-start", background: mine ? C.gold : C.card, color: mine ? "#141414" : C.white, border: mine ? "none" : `1px solid ${C.line}`, borderRadius: 14, borderBottomRightRadius: mine ? 4 : 14, borderBottomLeftRadius: mine ? 14 : 4, padding: "9px 13px", maxWidth: "78%", fontSize: 13, lineHeight: 1.4 }}>
              {m.content}
            </div>
          );
        })}
      ...
      <div style={{ display: "flex", gap: 8, padding: "10px 14px", borderTop: `1px solid ${C.line}` }}>
