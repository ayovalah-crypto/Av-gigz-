...
/* ---------- Roles ---------- */
/* "artist" is the only role that browses & books.
   Every other role is a "provider" — posts gigs/services, gets booked, gets paid. */
const ROLES = [
  { key: "producer", icon: "🎚️", title: "Music Producer", label: "PRODUCER", desc: "List your services & links, receive bookings, get paid." },
  { key: "artist", icon: "🎤", title: "Artist", label: "ARTIST", desc: "Browse providers, request services, chat & book sessions." },
  { key: "sound_engineer", icon: "🎛️", title: "Sound Engineer", label: "SOUND ENGINEER", desc: "Offer mixing, mastering & studio/live sound services." },
  { key: "videographer", icon: "🎥", title: "Videographer", label: "VIDEOGRAPHER", desc: "Shoot & edit music videos, list your packages." },
  { key: "graphic_designer", icon: "🎨", title: "Graphic Designer", label: "GRAPHIC DESIGNER", desc: "Design cover art, posters & branding for artists." },
  { key: "content_creator", icon: "📱", title: "Content Creator / Influencer", label: "CONTENT CREATOR", desc: "Offer promo, content creation & influencer packages." },
  { key: "event_booker", icon: "🎫", title: "Show / Event Booker", label: "SHOW / EVENT BOOKER", desc: "List booking slots & connect artists to shows." },
];
...
function BottomNav({ active, onNav, role }) {
  const items = [
    ...
    { key: "post", label: isProvider(role) ? "Post a Gig" : "Browse", icon: "➕", center: true },
...
function RoleSelect({ go, setRole }) {
  return (
    <div style={{ padding: "50px 24px 24px", display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ color: C.white, fontWeight: 800, fontSize: 20, marginBottom: 6 }}>How will you use AV Gigz?</div>
      <div style={{ color: C.mute, fontSize: 13, marginBottom: 20 }}>Each role gets its own platform and tools.</div>
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 10 }}>
        {ROLES.map((r) => (
          <button
            ...
            style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 16, padding: 18, display: "flex", alignItems: "center", gap: 16, marginBottom: 12, cursor: "pointer", textAlign: "left", width: "100%" }}
          >
            <div style={{ fontSize: 26 }}>{r.icon}</div>
            <div>
              <div style={{ color: C.white, fontWeight: 700, fontSize: 14.5 }}>{r.title}</div>
              <div style={{ color: C.mute, fontSize: 11.5, marginTop: 3 }}>{r.desc}</div>
            </div>
          </button>
        ))}
      </div>
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 18px 10px" }}>
        <div style={{ position: "relative", background: `linear-gradient(135deg, #1c1c1c, #101010)`, border: `1px solid ${C.gold}55`, borderRadius: 16, padding: 18, marginBottom: 16, overflow: "hidden" }}>
          <div style={{ position: "absolute", top: 8, right: 10, fontSize: 40, opacity: 0.12 }}>💿</div>
          <svg width="100%" height="40" viewBox="0 0 300 40" style={{ position: "absolute", bottom: 0, left: 0, opacity: 0.3 }}>
            <polyline
              points="0,26 15,10 30,32 45,16 60,24 75,8 90,28 105,18 120,26 135,12 150,30 165,20 180,26 195,10 210,26 225,18 240,30 255,14 270,24 285,8 300,22"
              fill="none"
              stroke={C.gold}
              strokeWidth="2"
            />
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 18 }}>
          <StatCard label="Active Gigs" value={myGigs.length} icon="🎙️" />
          <StatCard label="Pending Orders" value={pendingCount} accent icon="⏳" />
        ...
        {!loading && bookings.length === 0 && (
          <div style={{ border: `1px dashed ${C.line}`, borderRadius: 14, padding: "26px 14px", textAlign: "center", marginBottom: 8 }}>
            <div style={{ fontSize: 26, opacity: 0.4 }}>🎧</div>
            <div style={{ color: C.faint, fontSize: 12, marginTop: 8 }}>No orders yet — share your gigs to start getting booked.</div>
          </div>
        )}
      ...
      <div style={{ padding: "16px 18px 0" }}>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 12, padding: "11px 14px", display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
          ...
          <input
            ...
            placeholder={isProvider(role) ? "Search artists..." : "Search providers, services..."}
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 18px 10px" }}>
        <div style={{ background: `linear-gradient(135deg, #1c1c1c, #101010)`, border: `1px solid ${C.line}`, borderRadius: 16, padding: 20, marginBottom: 22 }}>
          <div style={{ color: C.white, fontWeight: 700, fontSize: 17, lineHeight: 1.35 }}>
            {isProvider(role) ? <>Find artists looking for your sound</> : <>Find the perfect<br />provider for your next hit</>}
          </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 20px 20px" }}>
        ...
        {!loading && bookings.length === 0 && (
          <div style={{ color: C.mute, fontSize: 13, textAlign: "center", marginTop: 40 }}>
            {role === "artist" ? "No bookings yet — browse providers to book a session." : "No bookings yet — share your gigs to get booked."}
          </div>
        )}
        {bookings.map((b) => (
...
function MyProfile({ go, profile, setProfile, role, userId }) {
      ...
      <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "0 20px" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 20 }}>
            <Avatar size={84} emoji={roleMeta(role).icon} src={draft.avatar_url} />
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", padding: "20px 20px 0", textAlign: "center" }}>
      <Avatar size={84} emoji={roleMeta(role).icon} src={profile.avatar_url} />
      <div style={{ color: C.white, fontWeight: 800, fontSize: 18, marginTop: 12 }}>{profile.name}</div>
      <div style={{ color: C.gold, fontSize: 11.5, fontWeight: 700, letterSpacing: 1, marginTop: 4 }}>{roleMeta(role).label}</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", borderTop: `1px solid ${C.line}`, paddingTop: 4 }}>
        {[
          ...
          isProvider(role) ? { label: "My Gigs", screen: "myGigs" } : null,
...
export default function App() {
  ...
  useEffect(() => {
    if (profile) {
      fetchProducers();
      if (isProvider(profile.role)) fetchMyGigs();
    }
  }, [profile]);
  ...
  switch (current.screen) {
    ...
    case "profile":
      ...
      break;
    case "book":
      body = <BookSession go={go} producer={current.data} artistProfile={profile} onBooked={() => isProvider(role) && fetchMyGigs()} />;
      break;
    case "bookingSuccess":
